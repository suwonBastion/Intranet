			...
			<div class="row">
				...
				<div>
						<c:if test="${p.showprev }">
							<a href="cflist?page=${p.beginpage-1 }&pagesize=${p.pagesize}&key=${p.key}&word=${p.word}"/>&lt;</a>
						</c:if>
						<%-- <c:if test="${p.totalcnt>10 }"> --%>
						<c:forEach  var="i"  begin="${p.beginpage }" end="${p.endpage }"  >
							<a href="cflist?page=${i }&pagesize=${p.pagesize}&key=${p.key}&word=${p.word}">${i }</a>
						</c:forEach>
						<%-- </c:if> --%>
						<c:if test="${p.shownext }">
							<a href="cflist?page=${p.endpage+1 }&pagesize=${p.pagesize}&key=${p.key}&word=${p.word}"/>&gt;</a>
						</c:if>
					</div>	
