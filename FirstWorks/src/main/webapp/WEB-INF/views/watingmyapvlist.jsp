		...
		<div class="container">
			<div class="row justify-content-center">
				<div class="col-md-6 text-center mb-5">
					<h2 class="heading-section">결재받아야될것리스트</h2>
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
					<form action="/watingmyapv">
						<select name="key">
				...
				<div>
						<c:if test="${p.showprev }">
							<a href="watingmyapv?page=${p.beginpage-1 }&pagesize=${p.pagesize}&key=${p.key}&word=${p.word}"/>&lt;</a>
						</c:if>
						<%-- <c:if test="${p.totalcnt>10 }"> --%>
						<c:forEach  var="i"  begin="${p.beginpage }" end="${p.endpage }"  >
							<a href="watingmyapv?page=${i }&pagesize=${p.pagesize}&key=${p.key}&word=${p.word}">${i }</a>
						</c:forEach>
						<%-- </c:if> --%>
						<c:if test="${p.shownext }">
							<a href="watingmyapv?page=${p.endpage+1 }&pagesize=${p.pagesize}&key=${p.key}&word=${p.word}"/>&gt;</a>
						</c:if>
					</div>					
			<a href="/main">메인으로</a>
		</div>
