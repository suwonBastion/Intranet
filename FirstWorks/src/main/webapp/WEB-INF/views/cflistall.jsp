		...
		<div class="container">
			<div class="row justify-content-center">
				<div class="col-md-6 text-center mb-5">
					<h2 class="heading-section">결재완료리스트</h2>
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
					<form action="/cflistall">
						<select name="key">
						...
						<table class="table">
						 ...
						 <c:forEach items="${list}" var="l">						
								<tr>			
									...
									<td>${l.mem_nm }</td>
									<td><a href="/cfdetail?apv_no=${l.apv_no }"><span>${l.apv_vc_tit }</span></a></td>
									<td>휴가</td>
								</tr>													
						</c:forEach>						    	    
						  </tbody>
				...
				<div>
						<c:if test="${p.showprev }">
							<a href="cflistall?page=${p.beginpage-1 }&pagesize=${p.pagesize}&key=${p.key}&word=${p.word}"/>&lt;</a>
						</c:if>
						<%-- <c:if test="${p.totalcnt>10 }"> --%>
						<c:forEach  var="i"  begin="${p.beginpage }" end="${p.endpage }"  >
							<a href="cflistall?page=${i }&pagesize=${p.pagesize}&key=${p.key}&word=${p.word}">${i }</a>
						</c:forEach>
						<%-- </c:if> --%>
						<c:if test="${p.shownext }">
							<a href="cflistall?page=${p.endpage+1 }&pagesize=${p.pagesize}&key=${p.key}&word=${p.word}"/>&gt;</a>
						</c:if>
					</div>	
					<a href="/cflistall">전체목록</a>
			<a href="/cflist">내결제목록</a>
			<a href="/main">메인으로</a>
		</div>
