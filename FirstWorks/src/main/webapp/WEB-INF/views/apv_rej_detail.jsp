	...
	<div id="brej">        
        <div id="rej">          
                ...
                <div id="b1">   
                    ...
                    <c:if test="${(lev==1&&list.mem_stamp_one!=null)||(lev==2&&list.mem_stamp_two!=null&&list.mem_stamp_one==null)}">
                    	<a href="/delrej?apv_no=${list.apv_no }" class="con">삭제</a>
                    </c:if> 
                    <c:if test="${(lev==1&&list.mem_stamp_one!=null)||(lev==2&&list.mem_stamp_two!=null&&list.mem_stamp_one==null)}">
	                    <form action="/retry" method="post">	                    
	                    	<input type="hidden" value="<c:choose><c:when test="${list.mem_stamp_one!=null }">${list.mem_stamp_one} </c:when><c:otherwise>alt.jpg</c:otherwise></c:choose>" name="mem_stamp_one">
	                    	<input type="hidden" value="<c:choose><c:when test="${list.mem_stamp_two!=null }">${list.mem_stamp_two} </c:when><c:otherwise>alt.jpg</c:otherwise></c:choose>" name="mem_stamp_two">
	                    	<input type="hidden" value="<c:choose><c:when test="${list.mem_stamp_three!=null }">${list.mem_stamp_three} </c:when><c:otherwise>alt.jpg</c:otherwise></c:choose>" name="mem_stamp_three">
	                    	<input type="hidden" value="${list.apv_vc_txt }" name="apv_vc_txt">
	                    	<input type="hidden" value="${list.apv_vc_rjt }" name="apv_vc_rjt">
	                    	<input type="hidden" value="${list.apv_no }" name="apv_no">
	                    	<input type="submit" class="con" value="재결재">                    	
                    </form> 
					</c:if>                                                           
                </div>            
        </div>
